...
const defaultSettings = {
    ...
    currency: 'Ft',
    fields: {
        workday_startTime: true, workday_endTime: true, workday_startLocation: true,
        workday_endLocation: true, workday_carPlate: true, workday_startOdometer: true,
        workday_endOdometer: true, workday_breakTime: true, refuel_odometer: true,
        refuel_fuelType: true, refuel_price: true, refuel_paymentMethod: true, refuel_location: true,
    }
};
...
const mergeSettings = (savedSettings) => {
    ...
    if (savedSettings.fields) {
        for (const key in finalSettings.fields) {
            if (savedSettings.fields[key] !== undefined) finalSettings.fields[key] = savedSettings.fields[key];
        }
    }
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    await user.update({ lastLoginTime: new Date(), lastLoginLocation: req.ip });
...
app.get('/api/my-settings', authenticateToken, async (req, res) => {
    try {
        ...
        console.error('My-Settings Error:', error);
        res.status(500).json({ error: 'Failed to load company settings.' });
    }
});

const adminAndSettingsRouter = express.Router();
adminAndSettingsRouter.use(authenticateToken, (req, res, next) => {
    if (req.user.role !== 'admin' && req.user.role !== 'superadmin') return res.status(403).json({ error: 'Forbidden' });
    next();
});

adminAndSettingsRouter.get('/api/settings', async (req, res) => {
...
adminAndSettingsRouter.post('/api/settings', async (req, res) => {
    const { role, companyId } = req.user;
    ...
    try {
        if (newSettings.fields) {
            for (const key in newSettings.fields) { newSettings.fields[key] = newSettings.fields[key] === true; }
        }
...
adminAndSettingsRouter.get('/api/admin/companies', async (req, res) => {
...
app.use(adminAndSettingsRouter);
...
eventsRouter.get('/workday-events', async (req, res) => {
    try {
        ...
        if (endDate) {
            ...
            where.startTime = { ...(where.startTime || {}), [Op.lte]: end };
        }
        ...
        const events = await db.WorkdayEvent.findAll({ 
            where, 
            include: [{ model: db.User, as: 'User', attributes: ['username', 'realName'] }], 
            order: [['startTime', 'DESC']] 
        });
        res.json(events);
    } catch (error) {
        console.error('GET /workday-events error:', error);
...
eventsRouter.post('/workday-events', async (req, res) => {
    try {
        ...
        console.error('POST /workday-events error:', e);
...
eventsRouter.get('/refuel-events', async (req, res) => {
    try {
        ...
        const events = await db.RefuelEvent.findAll({ 
            where, 
            include: [{ model: db.User, as: 'User', attributes: ['username', 'realName'] }], 
        ...
        console.error('GET /refuel-events error:', e);
...
eventsRouter.post('/refuel-events', async (req, res) => {
    try {
        ...
        console.error('POST /refuel-events error:', e);
        res.status(500).json({ error: e.message });
    }
});
...
const findAndAuthorizeEvent = (modelName) => async (req, res, next) => {
    try {
        const { role, userId: currentUserId, companyId: currentUserCompanyId } = req.user;
        const event = await db[modelName].findByPk(req.params.id, { include: [{ model: db.User, as: 'User' }]});
        if (!event) return res.status(404).json({ error: 'Event not found.' });

        if (role === 'user' && event.userId !== currentUserId) return res.status(403).json({ error: 'Forbidden' });
        if (role === 'admin') {
            const user = event.User || await db.User.findByPk(event.userId);
            if (user.companyId !== currentUserCompanyId) return res.status(403).json({ error: 'Forbidden' });
        }
        req.event = event;
        next();
    } catch (error) {
        console.error(`Authorization error in ${modelName}:`, error);
...
eventsRouter.put('/workday-events/:id', findAndAuthorizeEvent('WorkdayEvent'), async (req, res) => { 
    try { 
        ...
        console.error('PUT /workday-events/:id error:', e); 
...
eventsRouter.put('/refuel-events/:id', findAndAuthorizeEvent('RefuelEvent'), async (req, res) => { 
    try { 
        ...
        console.error('PUT /refuel-events/:id error:', e); 
        res.status(500).json({error: 'Update failed'}); 
    } 
});
